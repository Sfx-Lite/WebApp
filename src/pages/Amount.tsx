...
export default function Amount() {
  ...
  const debouncedAmount = useDebouncedValue(amountInput, 350);
  ...
  const isValid = enteredAmount > 0 && total <= availableBalance;
  ...
  const handleSendMax = () => {
    const feeAtFullBalance = feeData?.fee ?? 0;
    setAmountInput(Math.max(0, availableBalance - feeAtFullBalance).toFixed(2));
  };
      ...
      <div className="space-y-[2rem]">
        ...
        <div className="w-full md:max-w-[50%] mx-auto space-y-6">
          ...
          <div className="text-center space-y-1">
            ...
            {enteredAmount > 0 && (
              <p className="text-[13px] text-sfx-muted pt-1">
                {isFeeLoading
                  ? "Calculating fee…"
                  : (
                      <>
