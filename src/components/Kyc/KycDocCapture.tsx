import type { OcrData } from "@/store/kycSlice";
...
export default function KycDocCapture() {
  ...
  const documentType = useAppSelector(
    state => state.kyc.documentType,
  );

  const [capturedImage, setCapturedImage]
    = useState<string | null>(null);

  const [isPdf, setIsPdf]
    = useState(false);
  ...
  const [docDetected, setDocDetected]
    = useState(false);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (meanBrightness < 45) {
        ...
        setStatusMessage(
          "Lighting too dark — move to a brighter spot",
        );
      }
        ...
        setStatusMessage(
          "Position document card clearly in frame",
        );
      }
      else {
        setDocDetected(true);
  ...
  const handleDocumentProcess
    = async (
      ...
      let ocrResult: OcrData | null
        = null;

      if (!isPdfFile) {
        try {
          ocrResult
            = await processDocument(imageUrl);

          if (ocrResult) {
            toast.success(
              "Document text scanned successfully",
            );
          }
        }
        catch {
  ...
  const handleCapture
    = async () => {
      if (!docDetected) {
        toast.warning(
          "Please position your document within the bounding guide",
        );
  ...
  const handleFileUpload
    = (
      ...
      if (
        ...
        toast.error(
          "Unsupported format. Upload JPG, PNG, or PDF.",
        );
      ...
      if (
        ...
        toast.error(
          `File size exceeds limit (${MAX_FILE_SIZE_MB}MB).`,
        );
  ...
  const handleContinue = () => {
    if (!capturedImage) {
      toast.error(
        "Please capture or upload your document first",
      );
